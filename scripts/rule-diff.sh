...
done 3< <(jq -r '.repos[] | "\(.name) \(.url) \(.sha)"' "$CORPUS")
...
#
# The redirect is the fix; this is the check that would have named it. A
# harness whose own coverage is silent cannot be used to audit anything else.

...
func builtinAIRules() []*rules.Rule {
	defs := []aiRule{
		...
		// -----------------------------------------------------------------
		{
			...
			// and it reaches a multi-line call where widening the pattern would
			// not.
			//
			...
			//
			// `(?:^|...)` with the `m` flag is the second half. A character
			// class needs a character, so a bare `pipeline("sentiment-analysis")`
			// as the first token on a line -- the ordinary notebook idiom, and
			// notebooks are half this rule's file patterns -- matched nothing.

...
func TestApplicabilityClimbsAsFarAsTheEvidenceGoes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			v := applicabilityFor(tc.pkg, tc.adv, tc.linked, tc.linkedKnown, &sourceImports{}, nil, "")
...
func TestOnlyTheUnlinkedCaseIsNotImpacting(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		v := applicabilityFor(Package{Name: module, Ecosystem: tc.eco}, tc.adv, tc.linked, tc.linkedKnown, &sourceImports{}, nil, "")
...
func TestTheMessageSaysWhatWasEstablished(t *testing.T) {
	...
	unlinked := applicabilityFor(Package{Name: module, Ecosystem: "go"},
		goAdvisory(module, "golang.org/x/crypto/ssh"), linked, true, &sourceImports{}, nil, "")
	...
	stalled := applicabilityFor(Package{Name: module, Ecosystem: "go"},
		goAdvisory(module, "golang.org/x/crypto/openpgp"), linked, true, &sourceImports{}, nil, "")

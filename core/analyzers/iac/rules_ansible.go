...
func builtinAnsibleRules() []rules.Rule {
	defs := []iacRule{
		...
		},
		{
			...
			// The remediation follows the name, and for the same reason the name
			// changed. It told every reader to run `ansible-vault encrypt_string`
			// — advice that does not apply to the Kubernetes Secrets and the
			// Cassandra config the widening was prompted by, and the comment
			// above already argues at length that this is not an Ansible rule.
			// Ansible Vault stays, as one mechanism among several rather than the
			// only one named.

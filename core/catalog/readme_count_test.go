...
//
// It said 1506 across five suites, 938 secrets, 50 AI and 500 IaC, while the
// catalog held 1491, 882, 55 and 482. Nobody wrote a wrong number: every rule
// retirement since made the sentence a little less true — and the AI count had
// drifted the OTHER way, which is the direction nobody thinks to check. A count
// is exactly the kind of statement that goes on reading as current long after
// it stopped being so.
//
...
func TestREADMERuleCountsMatchTheCatalog(t *testing.T) {
	...
	claims := []struct {
		...
		{regexp.MustCompile(`ships with \*\*(\d+) built-in rules`), len(cat), "the total"},

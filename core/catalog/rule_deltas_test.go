...
//
// With one exception, and it is the case that showed the rule above has a hole.
// A `rule-removed` entry names an ID deliberately in neither set: the rule is
// gone and nothing absorbed it. It still matches a drop, because the harness
// diffs the BASELINE binary against the candidate and the baseline still has the
// rule — the drop is its count falling to zero. So "belongs to no rule,
// therefore can never match a drop" is false precisely for a removal, and the
// classification is what distinguishes the two.
//
// The check therefore runs in both directions: a `rule-removed` entry must name
// an ID that is NOT live, because an entry claiming the removal of a rule still
// in the catalog is as wrong as a typo and fails the same way.

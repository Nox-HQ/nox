...
func renderCollapses(b *strings.Builder, report *RuleReviewReport) {
	...
	if withheld := report.CollapsesMeasured - len(report.Collapses); withheld > 0 {
		...
		fmt.Fprintf(b, "AND with at least %d duplicated lines. The factor is proportional, the copy\n",
			report.CollapseCopiesShown)
		b.WriteString("count absolute, and neither catches the other's case: a 2x built from four\n")
		b.WriteString("findings is one duplicate, and 673 findings over 667 sites is six.\n")
		fmt.Fprintf(b, "The other %d fail one bar or both; pass `--all` to see them. The cutoff is\n", withheld)
		b.WriteString("presentation, not adjudication: crossing it means worth a look, nothing more.\n\n")
	} else {
		fmt.Fprintf(b, "Showing all %d measured rows.\n\n", report.CollapsesMeasured)
	}
	if len(report.Collapses) == 0 {
		fmt.Fprintf(b, "No rule clears a factor of %g with at least %d duplicated lines; pass `--all`\n",
			report.CollapseFactorShown, report.CollapseCopiesShown)
		fmt.Fprintf(b, "to see the %d that collapse by less.\n\n", report.CollapsesMeasured)
		return
	}
	fmt.Fprintf(b, "| %-10s | %8s | %8s | %7s | %6s | %6s |\n", "rule", "findings", "authored", "factor", "copies", "repos")
	fmt.Fprintf(b, "|%s|%s|%s|%s|%s|%s|\n", strings.Repeat("-", 12), strings.Repeat("-", 10),
		strings.Repeat("-", 10), strings.Repeat("-", 9), strings.Repeat("-", 8), strings.Repeat("-", 8))
	for _, c := range report.Collapses {
		fmt.Fprintf(b, "| %-10s | %8d | %8d | %7.3f | %6d | %6d |\n",
			c.Rule, c.Findings, c.Authored, c.Factor, c.Findings-c.Authored, c.Repos)
	}
	b.WriteString("\n")
}
